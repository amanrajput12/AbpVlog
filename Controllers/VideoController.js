...
export const AddVideo = async function(req,res){
    try {
       const {id,startTime,endTime,snippet,cardNo,Amount}  = req.body 
...
export const GetVideo = async function(req,res){
    ...
    const {email,name,acessToken,cardNo} = req.body
    console.log("current time",time,"card no ",typeof cardNo);
    
    try {
        const getdata = await Video.find({CardNo:cardNo,  startTime: { $lt: time }, endTime:{$gt:time}});
       console.log(getdata);
