...
const VerifyUser = () => {
  ...
  async function handledelete(value) {
    ...
    e.stopPropagation();
    console.log("click on the handledelete", value);
          ...
          <table className="bg-white border border-gray-300">
            <thead>
              <tr className="bg-gray-200 text-gray-900 text-sm xl:text-xl uppercase leading-normal">
                ...
                <th className="hidden xl:table-cell p-1 xl:py-3 xl:px-6 text-left">isVerified</th>
                <th className="p-1 xl:py-3 xl:px-6 text-left">Date</th>
                <th className="p-1 xl:py-3 xl:px-6 text-left">Button</th>
              </tr>
            </thead>
            <tbody className="text-gray-800 text-sm xl:text-3xl font-light">
              {filteredUsers?.map((user, i) => (
                <tr
                  onClick={() => {
                    dispatch(userVerfication({
                      userId: user._id,
                      email: user.email,
                      username: user.username,
                      Photo: user.paymentPhoto,
                ...
                >
                  <td className="p-1 xl:py-3 xl:px-6 text-left whitespace-nowrap">{i + 1}</td>
                  ...
                  <td className="xl:py-3 xl:px-6 text-left">
                    <button
                      onClick={(e) => handledelete({ e, deleteId: user._id })}
                      className="p-2 rounded-md bg-green-600 hover:bg-green-300"
                    >
                      Delete
                    </button>
                  </td>
